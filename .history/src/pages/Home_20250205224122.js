    ...
    <div className="relative min-h-screen w-screen">
      ...
      <div>
        {bubblePositions.map((position, index) => (
          ...
          >
            <motion.span className="absolute bg-gray-800 text-white text-xs px-2 py-1 rounded opacity-0 transition-opacity duration-300" whileHover={{opacity:1}}>{skills[index]}</motion.span>
          </motion.div>
        ))}
      </div>
    </div>
